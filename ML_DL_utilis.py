    ...
    def plotHistory(self, history, n = [1, 2], size = (5,5), show = False, prefix = ""):
        ...
        for metric in what:
            try:
                ...
                fig, ax1 = plt.subplots(1, 1, figsize=size, tight_layout=True, frameon=False)   
                #ax1.legend(loc='center left', bbox_to_anchor=(1, 0.5))
                if metric =="loss":
                    plt.semilogy(history["epochs"], history[metric], color=colors[n[0]], label='Train Loss' + label)
                    if 'val_'+metric in history:
                      plt.semilogy(history["epochs"], history['val_'+metric], color=colors[n[1]], label='Val Loss' + label, linestyle="--")

                elif metric =="lr":
                    plt.plot(history["epochs"], history['lr'], color=colors[n[0]], label= metric)
                ...
                plt.savefig(f"{x}{prefix}{metric}.png", bbox_inches = "tight")
                print(f"{x}{prefix}{metric}.png")
            except:
                continue
            if show == False:
                plt.close()
                
    def plotHistory1(self, history, n = [1, 2], size = (5,5), show = False, prefix = ""):
        colors = plt.rcParams['axes.prop_cycle'].by_key()['color']
        what = []
        for k in history.keys():
            if 'val' not in k:
    ...
    def make_confusion_matrix(self,
        ...
        if sum_stats:
            ...
            if len(cf)==2:
                ...
                resss = (accuracy, precision_recall_fscore_support(y, y_pred, average=None, labels = range(len(categories))))
                #print(resss)
        else:
            stats_text = ""
